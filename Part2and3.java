...
/**
 * Part 2 and 3 of Java Review
 * Pt 2 Runs a formatted list of all accounts to the console
 * Pt 3 Runs a user directed menu of account navigation
 * @author Kim Harvey
 *
 */
public class Part2and3 {
	...
	public static void main(String[] args) {
		// Part 2. Create customers and payments arrays
		final String[] customers = new String[] {"Amy Garrison","Braydon Beil","Bluebell Nguyen",
				"Konrad Sears","Cheryl Salas","Rosanna Meyer","Karis Haley","Enzo Donaldson",
				"Catrina Griffiths","Gregor Payne","Jermaine Jordan","Zakariya Goulding",
				"Mehdi Vickers","Zarah Lozano","Mattie Watkins","Hudson Sheridan","Rahul Tate",
				"Bruno Jefferson","Shaunie Mccallum","Mica Hendrix","Coco Houston","Kayla Stanton",
				"Noel Cooper","Tommie Leonard","Naima Bray","Harley Neville","Sabah Rosas",
				"Kade Mccartney","Bruce Valenzuela","Elif Ireland","Nansi Joseph","KeirBetts",
				"Harvie Barnard","Kady Whitworth","Sadiyah Levine","Sakina Cresswell","Chantel Wu",
				"Harriette Booth","Isla-Mae Edge","Daniela Shepherd","Cosmo Lam","Myron Archer",
				"Caspian Bradford","Elyse Fletcher","Halima Devlin","Humzah Madden","Bevan Donnelly",
				"Piotr Rice","Malikah Mejia","Mujtaba Bassett"};
		final double [][] payments = new double[][] {
		...
		//print header	
		header();
		//print report & check for number of zero payments
		runReport(customers, payments);
		System.out.println("\n");
		
		//Part 3
		//menu for various account referencing
		int select = 0; 
		while(select != 4) {
			menu();
			try {
				select = in.nextInt();
				switch(select) {
					case 1: custAcct(payments, customers);
						break;
					case 2: missedPayment(payments, customers);
						break;
					case 3: closedAcct(payments, customers);
						break;
					case 4: goodbye();
						break;
					default: System.out.println("Error in selection.");
						break;
				}
			} catch(Exception e) {
	...
	/**
	 * Find accounts with more than two missed payments and display report
	 * @param payments
	 * @param customers
	 */
	private static void closedAcct(double[][] payments, String[] customers) {
		System.out.println("\nClosed Accounts: ");
		boolean [] closed = new boolean[50];
		...
		runReport(customers, payments, closed);
	}
	...
	/**
	 * Find accounts with one or more missed payments and display report
	 * @param payments
	 * @param customers
	 */
	private static void missedPayment(double[][] payments, String[] customers) {
		System.out.println("\nAccounts with Missed Payments: ");
		boolean [] missed = new boolean[50];
		Arrays.fill(missed, false);
		...
		runReport(customers, payments, missed);
	}


	/**
	 * Runs payment history report and standing for one customer based
	 * off of a valid account number.
	 */
	private static void custAcct(double[][] payments, String[] customers) {
		int acct = 0;
		boolean valid = false;
		do {
			System.out.println("Please enter an account number: ");
			acct = in.nextInt();
			if((acct <= 1050) && (acct >= 1000)) {
				valid = true;
				break;
			} else {
		...
		for(int i = 0; i < payments.length; i++) {
			if(payments[i][0] == acct) {
				runReport(customers, payments, i);
			}
		}
	}
	
	/**
	 * Runs payment history report and standing for all customers
	 * @param customers
	 * @param payments
	 */
	private static void runReport(String[] customers, double[][] payments) {
		int count = 0;
		String status = "";
		
		for(int i = 0; i < customers.length; i++) {
			System.out.printf("%-20s", customers[i]);
			for (int j = 0; j< payments[0].length; j++) {
				if(payments[i][j]== 0) {
					count++;
				}
				System.out.printf("%3.0f ", payments[i][j]);
			}
			switch(count) {
				case 0: status = "Good";
					break;
	...
	/**
	 * Overloaded method to run payment history and print list of accounts with missed payments
	 * @param customers
	 * @param payments
	 * @param missed
	 */
	private static void runReport(String[] customers, double[][] payments, boolean[] missed) {
		int count = 0;
		String status = "";
		header();
	...
	/**
	 * Overloaded report method. Runs report for one particular customer.
	 */
	private static void runReport(String[] customers, double[][] payments, int num) {
		int count = 0;
		String status = "";
		header();
		System.out.printf("%-20s", customers[num]);
		for(int i = 0; i < payments[0].length; i++) {
			if(payments[num][i]== 0) {
					count++;
			}
			System.out.printf("%3.0f ", payments[num][i]);
	...
	/**
	 * Creates payment history report header
	 */
